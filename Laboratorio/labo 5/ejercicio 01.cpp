...
int NoDeNodos(Arbol p) {
	int  contador = 0;

	if (p->izq != NULL) {
		contador += NoDeNodos(p->izq);
	}
	if (p->der != NULL) {
		contador += NoDeNodos(p->der);
	}		
	contador ++;
	return contador;
}

int SumaDeNodos(Arbol p) {
	int  contador = 0;

	if (p->izq != NULL) {
		contador += SumaDeNodos(p->izq);
	}
	if (p->der != NULL) {
		contador += SumaDeNodos(p->der);
	}
	contador+=p->info;
	return contador;
}
...
int main() {
	...
	do {
		...
		case 3:
			...
			break;
		case 4:
			cout << "El numero de nodos en el arbol es: " << NoDeNodos(arbol) << endl;
			break;
		case 5:
			cout << "La suma de sus nodos es: " << SumaDeNodos(arbol) << endl;
			break;
